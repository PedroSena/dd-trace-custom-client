...
function getHost (options) {
  ...
  return [hostname, port].filter(val => val)
                         .join(":")
}
...
function hasAmazonSignature (options) {
  ...
  if (options.headers) {
    ...
    if ([].concat(headers["authorization"])
          .some(startsWith("AWS4-HMAC-SHA256"))) {
      return true
    }
  }
  return options.path && options.path.toLowerCase()
                                .indexOf("x-amz-signature=") !== -1
}
